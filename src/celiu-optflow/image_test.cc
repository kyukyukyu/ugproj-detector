...
template<typename Scalar, int DEPTH, int SIZE>
ostream& operator<<(ostream& os, const MCImage<Scalar, DEPTH, SIZE>& img) {
  for (int r = 0; r < img.height(); ++r) {
    for (int c = 0; c < img.width(); ++c) {
      os << " " << img(c, r)[0];
      for (int i = 1; i < img.depth(); ++i) os << "," << img(c, r)[i];
    }
    os << endl;
  }
  return os;
...
TEST(ImageTest, TestImage) {
  ...
  Image<uint8_t> patch = img_gray.GetBlock(1, 1, 3, 2);
  DUMP_IMAGE("patch", patch);
  img_gray.SetBlock(0, 0, patch);
  DUMP_IMAGE("img_gray", img_gray);
...
//  img_gray.fill('a');
  for (int y = 0; y < img_gray.height(); ++y) {
    for (int x = 0; x < img_gray.width(); ++x) {
      img_gray(x, y).fill('a' + (x % 3) + 3 * (y % 2));
    }
  }
//  img_rgb.fill('b');
  for (int y = 0; y < img_rgb.height(); ++y) {
    for (int x = 0; x < img_rgb.width(); ++x) {
      img_rgb(x, y).fill('a' + (x % 3) + 3 * (y % 2));
    }
  }
  DUMP_IMAGE("img_gray", img_gray);
  DUMP_IMAGE("img_rgb", img_rgb);

  MCImage<uint8_t, 1> patch_gray = img_gray.GetBlock(1, 2, 3, 2);
  MCImage<uint8_t, 3> patch_rgb = img_rgb.GetBlock(2, 1, 4, 3);
  DUMP_IMAGE("patch_gray", patch_gray);
  DUMP_IMAGE("patch_rgb", patch_rgb);
  img_gray.SetBlock(0, 0, patch_gray);
  img_rgb.SetBlock(0, 0, patch_rgb);
  DUMP_IMAGE("img_gray", img_gray);
  DUMP_IMAGE("img_rgb", img_rgb);
