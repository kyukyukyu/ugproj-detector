...
class Image : public Eigen::Array<SC, WIDTH, HEIGHT> {
 public:
  ...
  Image GetBlock(int x, int y, int w, int h) const {
    Image ret;
    ret.BaseType::operator=(this->block(x, y, w, h));
    return ret;
  }

  void SetBlock(int x, int y, const Image& img) {
    this->block(x, y, img.width(), img.height()) = img;
  }
};
...
// The main benefit of using Eigen's 2D array is that element-wise operations
// on multi-channel image can be done naturally through Eigen's functions.
//
//  (0,0) +------------------+
//  ch:0  |                  |-+
//        |                  | |-+
...
class MCImage : public Eigen::Array<_Scalar, DEPTH, SIZE> {
 public:
  ...
  MCImage GetBlock(int x, int y, int w, int h) const {
    const int d = depth();
    MCImage ret(w, h, d);
    for (int r = 0; r < h; ++r) {
      ret.block(0, r * w, d, w) = this->block(0, x + (y + r) * width_, d, w);
    }
    return ret;
  }

  void SetBlock(int x, int y, const MCImage& img) {
    const int w = img.width(), h = img.height(), d = depth();
    for (int r = 0; r < h; ++r) {
      this->block(0, x + (y + r) * width_, d, w) = img.block(0, r * w, d, w);
    }
  }
...
typedef MCImage<uint8_t, Eigen::Dynamic> MCImageU8X;
typedef MCImage<uint16_t, Eigen::Dynamic> MCImageU16X;
typedef MCImage<uint32_t, Eigen::Dynamic> MCImageU32X;
typedef MCImage<short, Eigen::Dynamic> MCImageShortX;
typedef MCImage<float, Eigen::Dynamic> MCImageFloatX;
